...
class RestaurantRobot(Robot):
    """Handle data model on resutaurant."""
    ...
    @_hello_decorator
    def recommend_resutaurant(self):
        """Show restaurant recommended restaurant to the user."""
        new_recommend_resutaurant = self.ranking_model.get_most_popular()
        if not new_recommend_resutaurant:
            return None
        
        will_recommend_restaurants = [new_recommend_resutaurant]
        while True:
            template = console.get_template_dir_path('greeting.txt', self.speek_color)
            is_yes = input(template.substitute({
                'robot_name': self.name,
                'user_name': self.user_name,
                'restaurant': new_recommend_resutaurant
            }))
            ...
            if is_yes.lower() == 'n' or is_yes().lower == 'no':
                ...
                if not new_recommend_resutaurant:
                    break
                will_recommend_restaurants.append(new_recommend_resutaurant)

    @_hello_decorator
    def ask_user_favorite(self):
        """Collect favolite restaurant information from users."""
        while True:
